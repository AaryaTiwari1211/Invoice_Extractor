...
# File Path Function
# def extract_text_from_rockman_pdf(file_path):
...
def extract_text_from_rockman_pdf(file_path):
    with open(file_path, 'rb') as file:
        ...
        # Provide a default encoding if chardet.detect() fails
        if encoding is None:
            encoding = 'utf-8'
        ...
        try:
            ...
            reader = PyPDF2.PdfReader(io.BytesIO(decoded_pdf_data.encode('latin-1')))
            text = ''
...
def rockman_invoice_data(text):
    ...
    final = f"{buyer_order_no}|{invoice_no}|{dated}|{quantity}|{part_number}|{motor_vehicle_no}|{dispatched_through}|"
    return final


pdf_path = r"backend\extractor_website\extractor\extractorapp\er\rockman.pdf"
extracted_text = extract_text_from_rockman_pdf(pdf_path)
print(extracted_text)
invoice_data = rockman_invoice_data(extracted_text)
print(invoice_data)
